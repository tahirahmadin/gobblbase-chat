  ...
  useEffect(() => {
    const checkBookingConfiguration = async () => {
      ...
      try {
        ...
        console.log(`Agent ${businessId} has valid booking settings:`, hasValidSettings);
        setIsConfigured(hasValidSettings);
        setError(null);
      } catch (error) {
        console.error(`Error checking booking configuration for agent ${businessId}:`, error);
        setIsConfigured(false);
        setError("Unable to verify booking configuration. Please try again later.");
      } finally {
        setIsLoading(false);
      }
    };
  ...
  if (!isConfigured) {
    return (
      <div className="flex flex-col items-center justify-center p-8 bg-white rounded-lg shadow-sm text-center">
        ...
        <h3 className="text-lg font-medium text-gray-800 mb-2">Booking Not Available</h3>
